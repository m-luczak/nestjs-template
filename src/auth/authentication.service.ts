...
export class AuthenticationService {
  ...
  public async registerOAuthUser(profile) {
    try {
      const createdUser = await this.usersService.createOauthUser(profile);
      createdUser.password = undefined;
      return createdUser;
    } catch (error) {
      if (error?.code === PostgresErrorCode.UniqueViolation) {
  ...
  public async getOauthUser(thirdPartyId: string) {
    const user = await this.usersService.getByThirdPartyId(thirdPartyId);
